...
class PurchaseOrderMedicine(models.Model):
    ...
    monto_abonado = fields.Monetary(
        ...
        compute="_compute_abonos_caryvil",
        store=True,
        currency_field="currency_id",
    )
    ...
    def _get_caryvil_payments(self):
        ...
        return lines.matched_credit_ids.credit_move_id.payment_id | lines.matched_debit_ids.debit_move_id.payment_id
    ...
    def action_generar_factura_final_caryvil(self):
        ...
        if self.abono_status_label != "pagado":
            raise ValidationError(
                _("Aún falta abonar $%.2f para poder generar la factura final.") % self.saldo_abonos_pendiente
            )
        ...
        if not self.invoice_ids:
            ...
            if lineas_sin_recibir:
                raise ValidationError(
                    _(
                        "No se puede generar la Factura Final porque la mercadería "
                        "aún no ha sido recibida en inventario. "
                        "Valide primero el albarán de entrada mediante el botón inteligente "
                        "'Recepción' antes de facturar."
                    )
                )
        ...
        if bills_por_pagar:
            wizard = (
                ...
                .with_context(
                    ...
                    active_ids=bills_por_pagar.ids,
                )
                .create({})
            )
    ...
    def _check_pharmacy_vendor(self):
        for order in self:
            ...
            if partner and not (partner.is_pharmacy_vendor or partner.commercial_partner_id.is_pharmacy_vendor):
                raise ValidationError(
                    _('El proveedor "%s" no está catalogado como proveedor farmacéutico.') % partner.name
                )
        ...
        for order in self:
            ...
            for line in order.order_line:
                if line.product_qty <= 0:
                    raise ValidationError(
                        _("La cantidad del medicamento %s debe ser mayor a 0.") % line.product_id.name
                    )
