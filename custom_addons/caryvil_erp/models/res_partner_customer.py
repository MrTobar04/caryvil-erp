...
class ResPartnerCustomer(models.Model):
    ...
    caryvil_invoice_count = fields.Integer(string="N° Compras", compute="_compute_caryvil_purchase_stats")
    caryvil_total_spent = fields.Monetary(
        string="Total Comprado ($)", currency_field="currency_id", compute="_compute_caryvil_purchase_stats"
    )
    ...
    _sql_constraints = [("dui_unique", "unique(dui)", "Ya existe un cliente registrado con este número de DUI.")]
            ...
            if record.dui:
                ...
                if not re.match(r"^\d{8}-\d{1}$", record.dui):
                    raise ValidationError(
                        _("El DUI ingresado (%s) no es válido. Debe cumplir el formato 00000000-0.") % record.dui
                    )
