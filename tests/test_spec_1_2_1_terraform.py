...
class TestTerraformFileStructure:
    ...
    def test_required_file_exists(self, filename: str):
        ...
        assert target.exists(), f"Falta el archivo requerido por SPEC-1.2.1 §5: {filename}"
    ...
    def test_tfvars_real_not_committed(self):
        ...
        assert "*.tfvars" in gitignore_content, "El .gitignore debe excluir *.tfvars para proteger credenciales reales"

    def test_tfstate_excluded_in_gitignore(self):
        """*.tfstate debe estar excluido en .gitignore (SPEC-1.2.1 §3)."""
        gitignore_content = GITIGNORE_PATH.read_text(encoding="utf-8")
        assert (
            "*.tfstate" in gitignore_content
    ...
    def test_dot_terraform_excluded_in_gitignore(self):
        ...
        gitignore_content = GITIGNORE_PATH.read_text(encoding="utf-8")
        assert ".terraform/" in gitignore_content, "El .gitignore debe excluir el directorio .terraform/"
...
class TestVersionsTf:
    ...
    def test_required_terraform_version(self):
        ...
        assert ">= 1.5.0" in content, "versions.tf debe declarar required_version = '>= 1.5.0'"

    def test_render_provider_declared(self):
        """El provider render-oss/render debe estar declarado en required_providers."""
        content = (TERRAFORM_DIR / "versions.tf").read_text(encoding="utf-8")
        assert "render-oss/render" in content, "versions.tf debe declarar el provider source = 'render-oss/render'"

    def test_provider_version_pinned(self):
        """La versión del provider debe estar fijada con ~> 1.3.0."""
        content = (TERRAFORM_DIR / "versions.tf").read_text(encoding="utf-8")
        assert "~> 1.3.0" in content, "La versión del provider debe estar fijada con '~> 1.3.0'"
...
class TestProvidersTf:
    ...
    def test_provider_render_block_exists(self):
        ...
        assert 'provider "render"' in content, 'providers.tf debe contener el bloque provider "render"'

    def test_api_key_uses_variable(self):
        """La api_key debe referenciarse como variable, no estar hardcodeada."""
        content = (TERRAFORM_DIR / "providers.tf").read_text(encoding="utf-8")
        assert "var.render_api_key" in content, "providers.tf debe usar var.render_api_key para la autenticación"

    def test_owner_id_uses_variable(self):
        """El owner_id debe referenciarse como variable."""
        content = (TERRAFORM_DIR / "providers.tf").read_text(encoding="utf-8")
        assert (
            "var.render_owner_id" in content
...
class TestVariablesTf:
    ...
    def test_variable_declared(self, varname: str):
        ...
        assert f'variable "{varname}"' in content, f"variables.tf debe declarar la variable '{varname}'"
    ...
    def test_render_api_key_is_sensitive(self):
        ...
        assert re.search(pattern, content, re.DOTALL), "La variable render_api_key debe tener sensitive = true"
    ...
    def test_odoo_admin_password_is_sensitive(self):
        ...
        assert re.search(pattern, content, re.DOTALL), "La variable odoo_admin_password debe tener sensitive = true"
    ...
    def test_render_owner_id_is_sensitive(self):
        ...
        assert re.search(pattern, content, re.DOTALL), "La variable render_owner_id debe tener sensitive = true"

    def test_region_default_is_oregon(self):
        """La región por defecto debe ser 'oregon' según SPEC-1.2.1 §5."""
        content = (TERRAFORM_DIR / "variables.tf").read_text(encoding="utf-8")
        assert '"oregon"' in content, "El valor por defecto de la variable region debe ser 'oregon'"
...
class TestMainTf:
    ...
    def test_render_postgres_resource_exists(self):
        ...
        assert 'resource "render_postgres"' in content, "main.tf debe declarar el recurso render_postgres"

    def test_render_web_service_resource_exists(self):
        """main.tf debe declarar el recurso render_web_service."""
        content = (TERRAFORM_DIR / "main.tf").read_text(encoding="utf-8")
        assert 'resource "render_web_service"' in content, "main.tf debe declarar el recurso render_web_service"

    def test_postgres_plan_is_free(self):
        """El plan de PostgreSQL debe ser 'free' (SPEC-1.2.1 §5)."""
        content = (TERRAFORM_DIR / "main.tf").read_text(encoding="utf-8")
        assert re.search(r'plan\s*=\s*"free"', content), "El recurso render_postgres debe usar plan = 'free'"

    def test_postgres_version_is_16(self):
        """La versión de PostgreSQL debe ser '16' (ADR-0001)."""
        content = (TERRAFORM_DIR / "main.tf").read_text(encoding="utf-8")
        assert re.search(
            r'version\s*=\s*"16"', content
    ...
    def test_web_service_uses_docker_image(self):
        ...
        content = (TERRAFORM_DIR / "main.tf").read_text(encoding="utf-8")
        assert "image_url" in content, "render_web_service debe usar runtime_source.image.image_url"

    def test_web_service_references_ghcr_image(self):
        """La imagen Docker debe referenciarse desde GitHub Container Registry (GHCR)."""
        content = (TERRAFORM_DIR / "main.tf").read_text(encoding="utf-8")
        assert "ghcr.io" in content, "La imagen de Odoo debe provenir de GitHub Container Registry (ghcr.io)"

    def test_proxy_mode_env_var_set(self):
        """PROXY_MODE debe estar configurada como True (requerido detrás del proxy de Render)."""
        content = (TERRAFORM_DIR / "main.tf").read_text(encoding="utf-8")
        assert "PROXY_MODE" in content, "El Web Service debe tener la variable de entorno PROXY_MODE configurada"
    ...
    def test_admin_password_env_var_references_variable(self):
        ...
        content = (TERRAFORM_DIR / "main.tf").read_text(encoding="utf-8")
        assert (
            "var.odoo_admin_password" in content
    ...
    def test_no_hardcoded_credentials(self):
        ...
        for pattern in suspicious_patterns:
            assert not re.search(pattern, content), f"main.tf contiene credenciales hardcodeadas: {pattern}"
...
class TestOutputsTf:
    ...
    def test_service_url_output_exists(self):
        ...
        assert "odoo_service_url" in content, "outputs.tf debe declarar el output 'odoo_service_url'"

    def test_postgres_internal_host_output_exists(self):
        """outputs.tf debe declarar la cadena de conexión interna de PostgreSQL."""
        content = (TERRAFORM_DIR / "outputs.tf").read_text(encoding="utf-8")
        assert (
            "postgres_internal_host" in content or "postgres_host" in content
    ...
    def test_postgres_connection_output_is_sensitive(self):
        ...
        content = (TERRAFORM_DIR / "outputs.tf").read_text(encoding="utf-8")
        assert (
            "sensitive   = true" in content or "sensitive = true" in content
...
class TestTfvarsExample:
    ...
    def test_example_contains_key(self, key: str):
        ...
        content = (TERRAFORM_DIR / "terraform.tfvars.example").read_text(encoding="utf-8")
        assert key in content, f"terraform.tfvars.example debe documentar la clave '{key}'"
